            ...
            <div className="divide-y divide-slate-100">
              {tailors.length ? (
                tailors.map((tailor) => {
                  ...
                  const pendingPayable = openOrders.reduce(
                    (sum, order) => sum + asNumber(order.tailorRate),
                    0
                  );
                  ...
                  return (
                    <article className="bg-white p-5 transition hover:bg-slate-50" key={tailor.id}>
                      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_9rem_9rem_9rem]">
                        ...
                        <div>
                          <p className="text-xs font-medium text-slate-400">Delivered shown</p>
                          <p className="mt-1 text-sm font-semibold text-slate-950">
                      ...
                      <div className="mt-4 grid gap-3 lg:grid-cols-2">
                        {tailor.stitchingOrders.length ? (
                          tailor.stitchingOrders.slice(0, 6).map((order) => (
                            <Link
                              className="rounded-2xl border border-slate-100 bg-slate-50 p-3 transition hover:border-slate-200 hover:bg-white"
                            ...
                            >
                              <div className="flex items-start justify-between gap-3">
                                ...
                                <span
                                  className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${orderTone(order.status)}`}
                                >
                                  {order.status}
                                </span>
                              </div>
                          ...
                          <p className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-500 lg:col-span-2">
                            No stitching work assigned yet.
                          </p>
                        )}
                      </div>
                    </article>
